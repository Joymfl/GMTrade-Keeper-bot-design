:PROPERTIES:
:ID:       0d57b1d5-b566-49e6-8107-e159b2c943f5
:END:
#+title: #+title: GMTrade Final Proposal (Backend)
#+filetags:  
...
We won't tell you what functions the Keeper should include — figuring out what must exist vs. what can wait is part of the exercise.
...
If after reading the repo you think there's a different module more worth tackling (off-chain backend scope only), you may choose your own — but justify the choice at the top of your report.
...
** Why?
Because in my opinion for any on chain trading platform, it's the load bearing component. Also exercises most of the protocol, which would introduce me to the repo and it's subparts (Quite a challenge!)
 ...
 * Assumes external manager for roles provision and configs for these keepers. I.e ORDER_KEEPER
   
*** Architecture
**** Update ingress and execution
**** Machine for cache sizes and hardware topology = Ryzen 9950 X
   * NOTE
   ...
   * This will define the memory model first.
     SDK provides helpers to deserialize and cache hits. So, I'll focus on hardware layout.
***** Ingress
...
******* Geyser
    Listens to Market Accounts, uses the sdk to deserialize into MarketModel struct (Market struct is 9kb it would fit comfortably inside L2 cache \approx 1 mb per core. So L2 cache hit time is what I'm optimizing for here. Since L2 is per cpu, ideally the threading lands on the hyperthread on the same cpu as the main thread, otherwise L3 hits are fast enough too, within the same CCD*)
    Now MarketModel which the sdk deserializes for me, comes in around 80 bytes (Not sure what I can strip here, to reduce size, so taking 80 bytes as considretion). But the issue is with multiple relevant markets. Since The Keeper Fleet, divides responsibilites between keepers to reduce competition, I'd design around it, saying that one keeper has a "Hot Set" of markets to work with, and if I pass around the MarketModel, which holds a pointer to the market struct (Heap Alloc), then 80 bytes fits ~1.25 cachelines.

    Fetches relevant Position account and deserializes into PositionModel . Suggested improvement -> PositionModel copies MarketModel into it's struct. Could just use a Ref or Arc if shared between multiple threads. 
    ...
    Once the Struct is created, use a channel to send updates to main thread. Main thread then invalidates and updates it's own cache (if on seperate CPU), or just replaces value (within same CPU, given that the data fits in L2)
...
***** Main concerns here:
  - For the async tasks consuming data from geyser and price feeds, Ideally I'd measure how many times the syscalls trap into kernel, and what's the overhead there. Not adding it to the design since I don't have numbers, but if the overhead is high kernel bypass (AF_XDP) is a valid option.
...
#+end_src       
**** Transaction sending & reliability
***** Responsibilites
- Sending transactions using a transport layer that's performant and reliable (skewing towards reliability)
...
A better approach would be to send directly to leaders with a fanout strategy via a staked connection. I have worked with TPU_CLIENT_NEXT before for the anza client and have measured reliability and performance gains for the same. It abstracts the leader fanout strategy and QUIC connection management, for confirmation, we'd use a transaction confirmation task (async). [[https://github.com/rpcpool/yellowstone-jet/blob/b3873c4db22be9680a371d5a61662c3225262ea1/apps/jet/src/transactions.rs#L223][example]] . This avoids the extra hop, but increases infra maintainence requirements though, and if that needs to be avoided, going through RPC is preferable. 
...
I've also noticed an optimize() method in the sdk, that squashes transaction batches to make them smaller, which would be nice to have and is added as the last pass

Which looks like:
#+name: tpu-processing
#+begin_src plantuml :file-ext svg
title TPU Processing (Pre Egress)
- Execution Stage
start
fork
,* Liquidation Dequeue Drain 
    ,* Ingest signed blob and build transaction for Liquidation
    ,* optimize() batch
    ,* sign and send
    ,* Add to uncomfirmed transaction map (read by confirmation task)

fork again
,* Order Fill Dequeue drain
        ,* build transaction
        ,* optimize batch()
        ,* Sign and send
        ,* Add to uncomfirmed transaction map (read by confirmation task)
...
#+RESULTS: tpu-processing
[[file:tpu-processing.svg]]

#+RESULTS:
[[file:/tmp/babel-qRD8u8/plantuml-OBHnKa.png]]
...
#+begin_src plantuml :file-ext svg
@startuml       
title Transaction Egress
rectangle "Keeper-TPU" as Keeper
rectangle Leader1
rectangle Leader2
rectangle Leader3
...
- I'm genuinely split between making this off thread vs including this in hot path. Essentially the tradeoff I'm envisioning is this
  Same thread, hot cache on relevant accounts faster egress
  Off thread, main thread keeps computing as data comes in
  
  So, I'm going with a hybrid. Transaction egress on the main thread (building and sending). The confirmation and subsequent retries off thread in a task
  
- I havent' calculated instruction sizes, but by eyeballing I see a large number of accounts per instruction. If they spill into multi transaction batches (I see the sdk has Batch helpers and sequential confirm and send send_all() methods), but this opens up issues, where atomicity is not guaranteed. if there's a batch of 3, tx 2 and 3 could fail and leave my keeper in a confused state. For this, I would use Jito Bundles that provides atomicity to avoid this issue, at the cost of higher auction fees.
...
- Executor Stage. Main Thread. Data structure mentioned in the executor section stays in L2 cache, and updates in place. Optional Upgrades: I don't know what fields can be trimmed for the data structure for v1, otherwise could attempt L1. This sends triggered liquidation account data to Egress stage. 2 seperate queues based on whether they're order actions or liquidations
...
This needs a service running that polls mapped keepers on it's /health endpoint and also relevant metrics.
- It should poll /health. And if down, remove from internal list of keepers, and update configs of the remaining keepers to balance out the markets.
  A redundancy note that should be used in this case, is copy the markets twice. I.E "BTC / USD" would be copied between Keeper A and Keeper B. There would be some competition in that case, but in case a Keeper fails, there's a fallback
...
The idea is to use [[https://solana.com/docs/rpc/websocket/logssubscribe][logsSubscribe]] via geyser, and watch error rates (runtime errors only) and map to keepers. Using a modified token bucket algorithm, we can start maintaining a map =HashMap<Pubkey,u64>= of failure counts that if crosses a threshold, could remove ORDER_KEEPER role from said keeper. Or at least notify as sysadmin

*** Market Division.
The idea here is division of labor.
We get a list of markets as a rpc fetch (no need to stream, since this is low volume update), and hash markets based on identifier (maybe Market Key?), and use that as a round robin dispatch, when autogenerating config files =Hash(MarketKey)%keeper_count= . And this could be rebalanced, if the Detection section detects a keeper is down, to rebalance, updating keeper_count


* Hardest / Most overlooked part of this system
** Transaction reliability under stress
Although my suggestion of tpu client next, improves reliability of transactions hitting leader and via staked connection higher chance of getting my transactions processed, it doesn't deal with validator scheduler heurisitics at all. 
My naive solution to this, would be to maximize compute budget priority fees to have my transactions executed
** Bundle racing
The TPU Egress part of the keeper pipeline is a bit fragile. There's replication across 2 keepers for every market. But, bundles have no guaranteed atomicity, and between each transaction, is the possibility of failure, frontrunning, or another keeper racing for the same. 
...
This design choice stems from the fact, that hot path should not block on network requests. Hence, the ingress stage supplies the executor with prices to check for liquidation, which is then forwarded to the TPU stage, where it injects signed blobs. The reason for this is that the executor stage shouldn't block waiting for signed blob responses (network call), before processing next price mark.

The concern here is wasted cycles for scanning redundant accounts that could possibly be closed by transactions being sent. But, without numbers it's hard to tell if this is a concern of over optimization. Metrics defined in above section would be able to show if this is worth improving. 
...
The transaction confirmation task doesn't get a lot of description above, so I'll describe it here.
If a transaction succeeds, that's the happy path. We record relevant metrics (such as retries_needed) and remove that position / order entry from our cache.

On failure, this has some edge cases:
1. If failed due to account already closed by another keeper, don't retry and emit metrics of failure. (This could be optimized further by listening to geyser streams on processed or getting raw shreds and maintain a confidence metric for attempting a liquidation or order creation.)
...
3. Not valid liquidation, don't retry, and wait for streams to provide another opportunity
...
The state of the art read layer is [[https://blog.triton.one/inside-cloudbreak-indexing-architecture-for-performant-account-reads/][Cloudbreak]] By trition which fits our needs very well, which uses Postgres as the DB.
...
#+begin_src plantuml :file-ext svg
  @startuml
  title Order Keeper — Data Flow (single keeper, pinned to one CCD)
start
- Ingress
fork
        - Process Geyser
        - Feed Channel (Consumer = Executor stage)
fork again
        - Process Pyth (Hermes)
        - Feed Channel (Consumer = Executor stage)
...
fork
    - Drain Price channels and New Positions (main thread)
    :Check Markets in price update;
    :Hash pair name/key;
    :Fetch and sweep against DS mentioned in executor stage;
    :Push to Liquidity channel for TPU stage;
fork again
    - Drain new orders channels (off main thread)
    :fetch necessary accounts from update;
    :Push to Order channel for TPU stage;
end merge
- TPU Stage
fork
     - Drain Liquidity Channel
     - Build transaction
     - Fetch signed blob and insert
     - Sign and send
fork again
     - Drain Order Channel
     - Build transaction
...
#+RESULTS: data-flow-diagram
[[file:data-flow-diagram.svg]]
* Other aspects considered but not explored
** Parsing data
In the sdk and the helpers, I've noticed parsing and deserializing uses a lot of allocations, which isn't great on the hot path. Would like to explore further zero copy mechanisms if this is a bottleneck in the keeper
